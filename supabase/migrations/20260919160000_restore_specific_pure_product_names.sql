WITH linked AS (
  ...
  SELECT
    ...
    CASE
      WHEN primary_row.source_name ~* '^Acrylic\s+[0-9]+MM\s+' THEN
        lower((regexp_match(primary_row.source_name, '^Acrylic\s+([0-9]+)MM\s+(.+)$', 'i'))[1])
        || 'mm Acrylic-'
        || (regexp_match(primary_row.source_name, '^Acrylic\s+([0-9]+)MM\s+(.+)$', 'i'))[2]
      WHEN primary_row.source_name ~* '^Acrylic\s+[0-9]+mm\s+' THEN
        lower((regexp_match(primary_row.source_name, '^Acrylic\s+([0-9]+)mm\s+(.+)$', 'i'))[1])
        || 'mm Acrylic-'
        || (regexp_match(primary_row.source_name, '^Acrylic\s+([0-9]+)mm\s+(.+)$', 'i'))[2]
      WHEN primary_row.source_name ~* '^Acrylic\s+' AND primary_row.family_key ~ '^ACR-([0-9]+)MM' THEN
        (regexp_match(primary_row.family_key, '^ACR-([0-9]+)MM', 'i'))[1]
        || 'mm Acrylic-'
        || regexp_replace(primary_row.source_name, '^Acrylic\s+', '', 'i')
      WHEN primary_row.source_name ~* '^Rhinestones\s+.+\s+[0-9]+MM$' THEN
        (regexp_match(primary_row.source_name, '^Rhinestones\s+(.+)\s+([0-9]+)MM$', 'i'))[2]
        || 'mm Rhinestones '
        || (regexp_match(primary_row.source_name, '^Rhinestones\s+(.+)\s+([0-9]+)MM$', 'i'))[1]
  ...
  WHERE primary_row.name IN (
    '15mm Bead', '12mm Bead', '16mm Bead', '20mm Bead',
    '16mm Acrylic Bead', '20mm Acrylic Bead', '16mm Bubblegum Bead',
    '20mm Bubblegum Bead', '16mm Printed Bead', '20mm Bead',
    'Printed Bead 15mm Round Bead', 'Acrylic Flatback', 'ilicone Round Bead'
  )
), updates AS (
  SELECT
    id,
    ...
    seo_title,
    search_text,
    CASE WHEN name_without_suffix ~* '\s+Bead$' THEN name_without_suffix ELSE name_without_suffix || ' Bead' END AS display_name
  FROM proposed
)
UPDATE products p
SET
