...
create temp table canonical_products on commit drop as
select distinct product_id
from canonical_sku_values;

create temp table canonical_sku_options on commit drop as
select product_id, gen_random_uuid() as id
from canonical_products;

delete from public.product_options
where product_id in (select product_id from canonical_products);
...
join canonical_sku_options options using (product_id);
