create or replace function public.storefront_sales_frozen()
returns boolean
language sql
stable
security definer
...
as $$
  select coalesce((select (value->'config'->>'salesFrozen')::boolean from public.site_settings where key = 'store' limit 1), false);
$$;

create or replace function public.etsy_imports_frozen()
returns boolean
language sql
stable
security definer
set search_path = public, private
as $$
  select coalesce((select (value->'config'->>'etsyImportsFrozen')::boolean from public.site_settings where key = 'store' limit 1), false);
$$;

revoke all on function public.storefront_sales_frozen() from public;
grant execute on function public.storefront_sales_frozen() to anon, authenticated, service_role;
revoke all on function public.etsy_imports_frozen() from public;
grant execute on function public.etsy_imports_frozen() to anon, authenticated, service_role;

create or replace view public.storefront_store_controls as
select
  public.storefront_sales_frozen() as sales_frozen,
  public.etsy_imports_frozen() as etsy_imports_frozen;

grant select on public.storefront_store_controls to anon, authenticated;

drop policy if exists orders_self_insert on public.orders;
create policy orders_self_insert on public.orders
  for insert to authenticated
  with check ((select auth.uid()) = user_id and not public.storefront_sales_frozen());
