...
create index if not exists orders_shipping_state_idx
  on public.orders ((upper(shipping_address->>'state')));
...
begin
  ...
  insert into public.orders (
    user_id, status, subtotal, discount, total, shipping_amount, tax_amount, tax_rate,
    tax_state, tax_jurisdiction, promo_code, shipping_name, shipping_address, carrier
  ) values (
    current_user_id, 'paid', subtotal, discount, total, shipping_amount, tax_amount, tax_rate,
