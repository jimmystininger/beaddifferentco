...
begin
  ...
  loop
    ...
    select coalesce(inventory.source_metadata->'product_pages'->p_product_id::text, '{}'::jsonb)
      into existing_page
    ...
    set source_metadata = jsonb_set(
          ...
          jsonb_build_object('options', answer_options)
            || case
                 when jsonb_typeof(existing_page) = 'object'
                   and nullif(trim(existing_page->>'image_url'), '') is not null
                 then jsonb_build_object('image_url', existing_page->'image_url')
                 else '{}'::jsonb
               end,
          true
        ),
