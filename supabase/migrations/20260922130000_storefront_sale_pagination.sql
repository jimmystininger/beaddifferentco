create index if not exists products_visible_promo_added_idx
  on public.products(visible, promo_starts_at, promo_ends_at, added_at desc, id desc)
  where visible = true and promo_discount_percent > 0;

create or replace function public.get_storefront_sale_products(
...
as $$
  with filtered as (
    ...
    where p.visible
      ...
      and (coalesce(p.promo_discount_percent, 0) > 0
        or (p.promo_price is not null and p.promo_price > 0 and p.promo_price < p.price))
  ), paged as (
...
revoke all on function public.get_storefront_sale_products(integer, integer)
  from public, anon, authenticated;
grant execute on function public.get_storefront_sale_products(integer, integer)
