alter table public.orders add column if not exists refunded_shipping_amount numeric not null default 0 check (refunded_shipping_amount >= 0);
...
begin
  ...
  if refund_shipping and shipping_remaining>0 then
    shipping_refund:=shipping_remaining; refund_total:=refund_total+shipping_refund;
    insert into public.order_financial_events(order_id,event_type,amount,reason,metadata,created_by) values(order_row.id,'refund',shipping_refund,nullif(trim(reason_value),''),jsonb_build_object('component','shipping'),auth.uid());
  end if;
  if refund_shipping and shipping_refund=0 and jsonb_array_length(line_items)=0 then raise exception 'Shipping has already been fully refunded.'; end if;
