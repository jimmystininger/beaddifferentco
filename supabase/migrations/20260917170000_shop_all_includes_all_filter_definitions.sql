      ...
      where v.filter_id = d.id and v.active
        and exists (
          ...
          where a.filter_value_id = v.id
            and (p_category_slug in ('', 'shop-all') or p.category_slug = p_category_slug or p.subcategory_slug = p_category_slug or exists (select 1 from public.product_categories pc where pc.product_id = p.id and pc.category_slug = p_category_slug))
        )
    )
