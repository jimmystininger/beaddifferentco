...
Register `https://zejcuqhihbfpuwsjvmhc.supabase.co/functions/v1/etsy-connect/callback` exactly as the redirect URI in Etsy. After the frontend is released, use **Store Control → Etsy connection → Connect Etsy**. The callback returns to the admin origin that initiated the connection. Access is read-only (`shops_r listings_r transactions_r`). **Store Control → Etsy connection** contains the ongoing order/listing imports, the recent fee resync, and staged import history. The completed historical fee backfill remains in the imported sales data. Order imports write revenue, discounts, shipping, tax, Etsy payment fees, refunds, and sale lines; they use saved Etsy SKU mappings for guarded inventory adjustments. Re-importing adjusts inventory only for a changed quantity. New-listing imports fetch the Etsy listing, inventory, photos, description, tags, materials, and category, create or reuse the canonical product and inventory SKU records, generate the matching 1PK SKU mapping, and leave created product pages inactive. Each scan records its result and links newly created pages directly to the admin editor for final review and activation. Review imports use an exact Etsy listing-to-product mapping and save the matched review as an ordinary approved item review, including its Etsy review photo when available. No review imports until its listing has one exact product match.
