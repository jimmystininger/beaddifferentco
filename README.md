...
The backend schema and RLS policies are recorded in `supabase/migrations/`. The browser uses the project URL and publishable key only; privileged keys must remain server-side. The configured live Supabase project has applied the audit migrations through `20260920161003_revoke_legacy_storefront_category_rpc`: the security hardening migration restores anonymous catalog RPC execution and removes write-class grants from the storefront controls view, the analytics migration adds SKU attribution, and the obsolete pre-v2 category RPC is no longer executable. The local migration directory also contains historical/superseded timestamp variants; do not replay or delete those files without reconciling the live ledger first.
